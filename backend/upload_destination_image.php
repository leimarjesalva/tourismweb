...
try {
    ...
    $target_filename = 'destination_' . time() . '_' . basename($file['name']);
    $target_path = $upload_dir . $target_filename;

    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        // also mirror the file to the frontend img/uploads folder so that static pages
        // can serve it directly without hitting the backend
