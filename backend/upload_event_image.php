...
// try to raise limits at runtime; host may still enforce lower ceilings
...
try {
    // check post_max_size so we can give nicer message instead of just empty $_FILES
    ...
    // also record upload_max_filesize for diagnostics
    ...
    // we can't fully enforce umax here because PHP handles it before script runs,
    // but we log it so admins know what the server limit is.
    ...
    error_log('DEBUG: Event upload request: ' . json_encode($debug_info));
    
    if (!isset($_FILES['image'])) {
        error_log('DEBUG: $_FILES[image] not set for event upload');
        j(['success' => false, 'error' => 'No file uploaded (FILE not found)', 'debug' => $debug_info]);
    }
    
    if (empty($_FILES['image']['tmp_name'])) {
        error_log('DEBUG: tmp_name empty for event upload; error code: ' . ($_FILES['image']['error'] ?? 'unknown'));
    ...
    error_log('DEBUG: Event file upload attempt - Name: ' . $original_name . ', MIME: ' . $mime . ', Ext: ' . $file_ext . ', Size: ' . $file['size']);
    // if the file somehow bypassed PHP size checking (rare), fail gracefully
    ...
    error_log('DEBUG: Event MIME check - mime_ok=' . ($mime_ok ? 'true' : 'false') . ', ext_ok=' . ($ext_ok ? 'true' : 'false'));
    
    if (!$mime_ok && !$ext_ok) {
        $error_detail = 'Invalid file type for event. Only images (JPEG, PNG, GIF, WebP) and MP4 videos allowed. File: ' . $original_name . ', MIME: ' . $mime . ', Ext: ' . $file_ext;
    ...
    $target_filename = 'event_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $ext;
    $target_path = $upload_dir . $target_filename;

    if (move_uploaded_file($file['tmp_name'], $target_path)) {
        error_log('DEBUG: Event file uploaded successfully: ' . $target_filename);
        // mirror to frontend
        ...
        error_log('DEBUG: move_uploaded_file failed for event: ' . $target_filename);
        j(['success' => false, 'error' => 'Upload failed']);
    }
} catch (Exception $e) {
    error_log('DEBUG: Event upload exception: ' . $e->getMessage());
